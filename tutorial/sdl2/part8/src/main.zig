...
//--- renderText
//---------------
fn renderText(game: *Game, text: []u8, x: c_int, y: c_int, color: Color) !void {
  const outlineColor = newColor(0, 0, 0, 0x8f);
  try renderTextSub(game.renderer, game.font, text, x, y, 2, outlineColor);
  try renderTextSub(game.renderer, game.font, text, x, y, 0, color);
}
...
fn render(alloc: std.mem.Allocator, self: *Game, tick: c_int) !void {
    ...
    const blue  = newColor(0x00, 0xff, 0xff, 0xff);
    ...
    if(time.begin >= 0){
        ...
        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf, "{s}{s}", .{"Finished in: ", try formatTime(alloc, time.finish)});
        try renderText(self, slc, 50, 100, white);
    }
    if(time.best >= 0){
        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf, "{s}{s}", .{"Best time  : " , try formatTime(alloc, time.best)});
    ...
    if(time.begin < 0){
        ...
        const colm = 30;
        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}", .{"Jump   : Space, Up, J, K"});
        try renderText(self, slc, 50, base+colm*1, white);

        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}",     .{"Left     : A, H, Left"});
        try renderText(self, slc, 50, base+colm*2, white);

        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}",     .{"Right   : D, L, Right"});
        try renderText(self, slc, 50, base+colm*3, white);

        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}",     .{"Restart: R"});
        try renderText(self, slc ,50, base+colm*4, white);

        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}",     .{"Quit     : Q, Esc"});
        try renderText(self, slc ,50, base+colm*5, white);

        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}{s}", .{"Zig-" , builtin.zig_version_string});
        try renderText(self, slc ,50, base+colm*7, white);

        clearBuf(&sbuf);
        var sdlVer:ig.SDL_version = undefined;
        ig.SDL_GetVersion(&sdlVer);
        slc = try std.fmt.bufPrint(&sbuf,"SDL2   : {d}.{d}.{d}", .{sdlVer.major, sdlVer.minor, sdlVer.patch});
        try renderText(self, slc, 50, base+colm*9, white);

        clearBuf(&sbuf);
        const ttfVer = ig.TTF_Linked_Version();
        slc = try std.fmt.bufPrint(&sbuf,"SDL_ttf: {d}.{d}.{d}", .{ttfVer.*.major, ttfVer.*.minor, ttfVer.*.patch});
        try renderText(self, slc, 50, base+colm*10, white);

        clearBuf(&sbuf);
        slc = try std.fmt.bufPrint(&sbuf,"{s}",     .{"Zig-Platformer-SDL2     "});
        try renderText(self, slc ,50, base+colm*14, blue);
    }
